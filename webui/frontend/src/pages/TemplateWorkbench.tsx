...
type WorkbenchItemRole = 'project' | 'template-folder' | 'template-file' | 'imported-file'
...
function projectCoverUrl(project: WorkbenchProjectIndex) {
  return project.cover ? `/api/template-workbench/projects/${encodeURIComponent(project.sequence)}/cover` : null
}
...
function createTemplateFolderItem(project: WorkbenchProjectIndex): TemplateWorkbenchItem {
  const templateName = templateNameForProject(project)
  return {
    id: `${project.sequence}:templates`,
    name: '部署模板',
    type: 'folder',
    role: 'template-folder',
    fileKind: 'folder',
    updatedAt: '已保存',
    sequence: project.sequence,
...
function CoverPreview({
  ...
  return (
    <aside
      ...
      style={{ left: 1585, top: 306, width: 285, height: 206.72, borderColor: 'var(--twb-card-border)', background: 'var(--twb-bg)', color: 'var(--twb-text)' }}
    >
      <div className="absolute inset-x-[-1px] top-[-1px] flex items-center justify-center overflow-hidden rounded-t-[10px]" style={{ height: 153, background: 'var(--twb-bg)' }}>
        {cover ? (
          <img src={cover} alt="模板封面" className="h-full w-full object-cover" />
        ) : (
          <Folder size={54} strokeWidth={1.2} className="opacity-30" />
        )}
      </div>
      <div className="absolute inset-x-[-1px] bottom-[-1px] rounded-b-[10px] border" style={{ height: 53.72, borderColor: 'var(--twb-card-border)', background: 'var(--twb-bg)' }} />
      <div className="absolute min-w-0" style={{ left: 7, right: 7, top: 162 }}>
        <div className="truncate leading-none" style={{ fontFamily: font, fontSize: 18, fontWeight: 700 }}>{title}</div>
        <div className="mt-[7px] leading-none" style={{ color: 'var(--twb-muted)', fontFamily: font, fontSize: 15, fontWeight: 300 }}>模板封面</div>
      </div>
    </aside>
  )
}
...
export default function TemplateWorkbench({
  ...
  const contentItems = useMemo(() => {
    ...
    if (route.type === 'templates') return [createTemplateFileItem(selectedProject)]
    return [
      createTemplateFolderItem(selectedProject),
  ...
  const openItem = async (item: TemplateWorkbenchItem) => {
    onOpenItem?.(item)
    if (item.role === 'template-folder' && item.sequence) {
      setRoute({ type: 'templates', sequence: item.sequence })
      return
    }
  ...
  const navigateBackInWorkbench = () => {
    if (route.type === 'templates' && selectedProject) {
      setRoute({ type: 'project', sequence: selectedProject.sequence })
      return
    }
  ...
  return (
    <div className="template-workbench relative h-full w-full overflow-hidden" style={{ background: 'var(--twb-bg)', color: 'var(--twb-text)' }}>
      ...
      <main className="absolute left-0 top-0 z-0 h-[1080px] w-[1920px]">
        ...
        {route.type === 'templates' && selectedProject && (
          <CoverPreview cover={projectCoverUrl(selectedProject)} title={selectedProject.mod_name || '部署模板'} />
        )}
